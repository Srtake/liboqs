...
const unsigned int popcount_param_iib[256] = {
0,1,1,2,1,2,2,3,
1,2,2,3,2,3,3,4,
1,2,2,3,2,3,3,4,
2,3,3,4,3,4,4,5,
...
static void rej_eta1_param_iib(uint32_t *a,const unsigned char *buf)
{
#if ETA1 > 3
...
static unsigned int rej_eta2_param_iib(uint32_t *a,unsigned int len,const unsigned char *buf)
{
#if ETA2 >7 || ETA2 <3
...
#endif
  rej_eta1_param_iib(a->coeffs,outbuf);
}
...
#error "rej_eta1() assumes ETA1 <=3"
  ...
  rej_eta1_param_iib(a0->coeffs,outbuf[0]);
  rej_eta1_param_iib(a1->coeffs,outbuf[1]);
  rej_eta1_param_iib(a2->coeffs,outbuf[2]);
}
                          ...
                          unsigned char nonce3)
{
    ...
    rej_eta1_param_iib(a1->coeffs,outbuf[1]);
    rej_eta1_param_iib(a2->coeffs,outbuf[2]);
    rej_eta1_param_iib(a3->coeffs,outbuf[3]);
}
                          ...
                          unsigned char nonce3)
{
    ...
    rej_eta2_param_iib(a0->coeffs,PARAM_N,outbuf[0]);
    rej_eta2_param_iib(a1->coeffs,PARAM_N,outbuf[1]);
    rej_eta2_param_iib(a2->coeffs,PARAM_N,outbuf[2]);
    rej_eta2_param_iib(a3->coeffs,PARAM_N,outbuf[3]);
}
                      ...
                      unsigned char nonce)
{
  ...
  rej_eta2_param_iib(a->coeffs, PARAM_N, outbuf);
#else
  for (i = 0; i < SEEDBYTES; ++i)
	  inbuf[i] = seed[i];
  inbuf[SEEDBYTES] = nonce;
  ...
  pos = rej_eta2_param_iib(a->coeffs, 223, outbuf);
  ...
  rej_eta2_param_iib(&a->coeffs[223], 33, &outbuf[pos]);
#else
#if ETA2 == 4
  unsigned char outbuf[3 * SHAKE256_RATE];
#else 
...
static void uniform_gamma1m1_param_iib(poly *a,unsigned char *buf)
{
#if GAMMA1 != 131072
#error "poly_uniform_gamma1m1() assumes GAMMA1 == 131072"
#endif
                              ...
                              uint16_t nonce2)
{
  ...
  uniform_gamma1m1_param_iib(a0,outbuf[0]);
  uniform_gamma1m1_param_iib(a1,outbuf[1]);
  uniform_gamma1m1_param_iib(a2,outbuf[2]);
}
                              ...
                              uint16_t nonce3)
{
  ...
  uniform_gamma1m1_param_iib(a1,outbuf[1]);
  uniform_gamma1m1_param_iib(a2,outbuf[2]);
  uniform_gamma1m1_param_iib(a3,outbuf[3]);
