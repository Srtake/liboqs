...
extern int msig_keygen_param_ii(uint8_t* pk, uint8_t* sk);
extern int msig_sign_param_ii(uint8_t* sk, uint8_t* m, size_t mlen, uint8_t* sm, size_t* smlen);
extern int msig_verf_param_ii(uint8_t* pk, uint8_t* sm, size_t smlen, uint8_t* m, size_t mlen);

OQS_API OQS_STATUS OQS_SIG_aigis_sig_param_ii_keypair(uint8_t* public_key, uint8_t* secret_key) {
    return (OQS_STATUS)msig_keygen_param_ii(public_key, secret_key);
}
OQS_API OQS_STATUS OQS_SIG_aigis_sig_param_ii_sign(uint8_t* signature, size_t* signature_len, uint8_t* message, size_t message_len, uint8_t* secret_key) {
    return (OQS_STATUS)msig_sign_param_ii(secret_key, message, message_len, signature, signature_len);
}
OQS_API OQS_STATUS OQS_SIG_aigis_sig_param_ii_verify(uint8_t* message, size_t message_len, uint8_t* signature, size_t signature_len, uint8_t* public_key) {
    return (OQS_STATUS)msig_verf_param_ii(public_key, signature, signature_len, message, message_len);
}
#endif