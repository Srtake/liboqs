...
void polyvecl_freeze2q_param_i(polyvecl *v) {
    ...
    for(i = 0; i < PARAM_L; ++i)
        poly_freeze2q_param_i(&v->vec[i]);
}

void polyvecl_freeze4q_param_i(polyvecl *v) {
    unsigned int i;  
    for(i = 0; i < PARAM_L; ++i)
        poly_freeze4q_param_i(&v->vec[i]);
}
...
void polyvecl_add_param_i(polyvecl *w, const polyvecl *u, const polyvecl *v) {
  ...
  for(i = 0; i < PARAM_L; ++i)
    poly_add_param_i(w->vec+i, u->vec+i, v->vec+i);
}
                                          ...
                                          const polyvecl *v) 
{
  ...
  poly_pointwise_invmontgomery_param_i(w, u->vec+0, v->vec+0);

  for(i = 1; i < PARAM_L; ++i) {
    poly_pointwise_invmontgomery_param_i(&t, u->vec+i, v->vec+i);
    poly_add_param_i(w, w, &t);
  }
  poly_barrat_reduce_param_i(w);
}

int polyvecl_chknorm_param_i(const polyvecl *v, uint32_t bound)  {
  unsigned int i;
  int ret = 0;

  for(i = 0; i < PARAM_L; ++i)
    ret |= poly_chknorm_param_i(v->vec+i, bound);
...
void polyveck_sub_param_i(polyveck *w, const polyveck *u, const polyveck *v) {
  ...
  for(i = 0; i < PARAM_K; ++i)
    poly_sub_param_i(w->vec+i, u->vec+i, v->vec+i);
}
...
void polyveck_invntt_montgomery_param_i(polyveck *v) {
  ...
  for(i = 0; i < PARAM_K; ++i)
    poly_invntt_montgomery_param_i(v->vec+i);
}

int polyveck_chknorm_param_i(const polyveck *v, uint32_t bound) {
  unsigned int i;
  int ret = 0;
...
void polyveck_power2round_param_i(polyveck *v1, polyveck *v0, const polyveck *v) {
  unsigned int i;
  for(i = 0; i < PARAM_K; ++i)
  	poly_power2round_param_i(&v1->vec[i],&v0->vec[i], &v->vec[i]);
}

void polyveck_decompose_param_i(polyveck *v1, polyveck *v0, const polyveck *v) {
  unsigned int i;
  for(i = 0; i < PARAM_K; ++i)
    poly_decompose_param_i(&v1->vec[i],&v0->vec[i],&v->vec[i]);
}
                                ...
                                const polyveck *v)
{
  unsigned int i,s = 0;
  for(i = 0; i < PARAM_K; ++i)
      s+=poly_make_hint_param_i(&h->vec[i],&u->vec[i],&v->vec[i]);
  
  return s;
}

void polyveck_use_hint_param_i(polyveck *w, const polyveck *u, const polyveck *h) {
  unsigned int i, j;
  poly v1,v0;
  for(i = 0; i < PARAM_K; ++i)
  {
   poly_decompose_param_i(&v1,&v0,&u->vec[i]);
    for(j = 0; j < PARAM_N; ++j)
    {
