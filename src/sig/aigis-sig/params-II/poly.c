...
void poly_freeze2q_param_ii(poly *a)
{
    int i;
...
void poly_freeze4q_param_ii(poly *a)
{
    int i;
    __m256i *pa  = (__m256i *) a->coeffs;
    __m256i q8x  = _mm256_set1_epi32(PARAM_Q);
...
void poly_barrat_reduce_param_ii(poly *a)
{
	int i;
    __m256i *pa  = (__m256i *) a->coeffs;
#ifndef _WIN64
...
void poly_decompose_param_ii(poly *r1,poly *r0, const poly *a)
{
  int i;
  __m256i * pa = (__m256i *) a->coeffs;
...
void poly_power2round_param_ii(poly *r1,poly *r0, const poly *a)
{
  int i;
  __m256i * pa = (__m256i *) a->coeffs;
  __m256i * pr0 = (__m256i *) r0->coeffs;
...
uint32_t poly_make_hint_param_ii(poly *h, const poly*a, const poly*b)
{
	int i;
	uint32_t s = 0;
	for (i = 0; i < PARAM_N; ++i)
...
void poly_add_param_ii(poly *c, const poly *a, const poly *b)  {
  unsigned int i;
...
void poly_sub_param_ii(poly *c, const poly *a, const poly *b) {
  unsigned int i;
...
void poly_shiftl_param_ii(poly *a, unsigned int k) {
  unsigned int i;
  __m256i * pa = (__m256i *) a->coeffs;
  for(i = 0; i < PARAM_N/8; ++i)
...
void poly_ntt_param_ii(poly *a) {
  ntt_param_ii(a->coeffs);
}

void poly_invntt_montgomery_param_ii(poly *a)
{
	invntt_param_ii(a->coeffs);
}
void poly_pointwise_invmontgomery_param_ii(poly *c, const poly *a, const poly *b)
{
  int i;
  __m256i * pa = (__m256i *) a->coeffs;
  __m256i * pb = (__m256i *) b->coeffs;
...
int  poly_chknorm_param_ii(const poly *a, uint32_t B)
{
	unsigned int i;
	__m256i * pa = (__m256i *) a->coeffs;
...
const unsigned int popcount_param_ii[256] = {
0,1,1,2,1,2,2,3,
1,2,2,3,2,3,3,4,
1,2,2,3,2,3,3,4,
2,3,3,4,3,4,4,5,
...
void poly_uniform_param_ii(poly *a, unsigned char *buf)
{
...
static void rej_eta1_param_ii(uint32_t *a,const unsigned char *buf)
{
#if ETA1 > 3
...
static unsigned int rej_eta2_param_ii(uint32_t *a,unsigned int len,const unsigned char *buf)
{
#if ETA2 >7 || ETA2 <3
...
void poly_uniform_eta1_param_ii(poly *a,
...
#error "rej_eta1() assumes ETA1 <=3"
  ...
  aes256ctr_squeezeblocks(outbuf, 2, &state); /* Probability we need more than 2 blocks: < 2^{-131}.*/
#else
...
#endif
  rej_eta1_param_ii(a->coeffs,outbuf);
}

void poly_uniform_eta1_3x_param_ii(poly *a0,
                          poly *a1,
                          poly *a2,
                          const unsigned char seed[SEEDBYTES],
...
#error "rej_eta1() assumes ETA1 <=3"
  ...
  rej_eta1_param_ii(a0->coeffs,outbuf[0]);
  rej_eta1_param_ii(a1->coeffs,outbuf[1]);
  rej_eta1_param_ii(a2->coeffs,outbuf[2]);
}

void poly_uniform_eta1_4x_param_ii(poly *a0,
                          poly *a1,
                          poly *a2,
                          poly *a3,
                          const unsigned char seed[SEEDBYTES],
                          ...
                          unsigned char nonce3)
{
    ...
    rej_eta1_param_ii(a1->coeffs,outbuf[1]);
    rej_eta1_param_ii(a2->coeffs,outbuf[2]);
    rej_eta1_param_ii(a3->coeffs,outbuf[3]);
}


void poly_uniform_eta2_4x_param_ii(poly *a0,
                          poly *a1,
                          poly *a2,
                          poly *a3,
                          const unsigned char seed[SEEDBYTES],
                          ...
                          unsigned char nonce3)
{
    ...
    rej_eta2_param_ii(a0->coeffs,PARAM_N,outbuf[0]);
    rej_eta2_param_ii(a1->coeffs,PARAM_N,outbuf[1]);
    rej_eta2_param_ii(a2->coeffs,PARAM_N,outbuf[2]);
    rej_eta2_param_ii(a3->coeffs,PARAM_N,outbuf[3]);
}
void poly_uniform_eta2_2x_param_ii(poly *a0,
	poly *a1,
	const unsigned char seed[SEEDBYTES],
	unsigned char nonce0,
	unsigned char nonce1)
...
void poly_uniform_eta2_param_ii(poly *a,
                      const unsigned char seed[SEEDBYTES], 
                      unsigned char nonce)
{
  unsigned int i;
  ...
  rej_eta2_param_ii(a->coeffs, PARAM_N, outbuf);
#else
  for (i = 0; i < SEEDBYTES; ++i)
	  inbuf[i] = seed[i];
  inbuf[SEEDBYTES] = nonce;
  ...
  pos = rej_eta2_param_ii(a->coeffs, 223, outbuf);
  ...
  rej_eta2_param_ii(&a->coeffs[223], 33, &outbuf[pos]);
#else
#if ETA2 == 4
  unsigned char outbuf[3 * SHAKE256_RATE];
#else 
...
void poly_uniform_gamma1m1_param_ii(poly *a,
...
static void uniform_gamma1m1_param_ii(poly *a,unsigned char *buf)
{
#if GAMMA1 != 131072
#error "poly_uniform_gamma1m1() assumes GAMMA1 == 131072"
#endif
...
void poly_uniform_gamma1m1_3x_param_ii(poly *a0,
                              poly *a1,
                              poly *a2,
                              const unsigned char seed[SEEDBYTES + CRHBYTES],
                              uint16_t nonce0,
                              ...
                              uint16_t nonce2)
{
  ...
  uniform_gamma1m1_param_ii(a0,outbuf[0]);
  uniform_gamma1m1_param_ii(a1,outbuf[1]);
  uniform_gamma1m1_param_ii(a2,outbuf[2]);
}
void poly_uniform_gamma1m1_4x_param_ii(poly *a0,
                              poly *a1,
                              poly *a2,
                              poly *a3,
                              const unsigned char seed[SEEDBYTES + CRHBYTES],
                              ...
                              uint16_t nonce3)
{
  ...
  uniform_gamma1m1_param_ii(a1,outbuf[1]);
  uniform_gamma1m1_param_ii(a2,outbuf[2]);
  uniform_gamma1m1_param_ii(a3,outbuf[3]);
  
}

void polyeta1_pack_param_ii(unsigned char *r, const poly *a) {
#if ETA1 > 3
...
void polyeta2_pack_param_ii(unsigned char *r, const poly *a) {
#if ETA2 > 7
...
void polyeta1_unpack_param_ii(poly *r, const unsigned char *a)
{
#if ETA1 > 3
...
void polyeta2_unpack_param_ii(poly *r, const unsigned char *a)
{
#if ETA2 > 7
...
void polyt1_pack_param_ii(unsigned char *r, const poly *a)
{
#if QBITS - PARAM_D != 8
...
void polyt1_unpack_param_ii(poly *r, const unsigned char *a)
{
#if QBITS - PARAM_D != 8
...
void polyt0_pack_param_ii(unsigned char *r, const poly *a) {
...
void polyt0_unpack_param_ii(poly *r, const unsigned char *a)
{
#if PARAM_D!=13 && PARAM_D!=14
#error "polyt0_unpack() assumes PARAM_D== 13 or 14"
#endif
...
void polyz_pack_param_ii(unsigned char *r, const poly *a) {
...
void polyz_unpack_param_ii(poly *r, const unsigned char *a) {
...
void polyw1_pack_param_ii(unsigned char *r, const poly *a)
{
#if PARAM_Q/ALPHA > 8
