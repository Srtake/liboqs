...
int msig_keygen(unsigned char *pk, unsigned char *sk) 
{
...
#elif PARAM_L==3
  poly_uniform_eta1_3x(&s1.vec[0],&s1.vec[1],&s1.vec[2],buf, nonce,nonce+1,nonce+2);
  nonce += 3;
#elif PARAM_L==4
  poly_uniform_eta1_4x(&s1.vec[0],
                         &s1.vec[1],
                         &s1.vec[2],
  ...
  poly_uniform_eta1_4x(&s1.vec[0],
	  ...
	  &s1.vec[3],buf,nonce,nonce+1,nonce+2,nonce+3);
  nonce += 4;
  poly_uniform_eta1(&s1.vec[4], buf, nonce++);
#endif
  ...
  poly_uniform_eta2_4x(&s2.vec[0],
                         ...
                         &s2.vec[2],
                         &s2.vec[3],buf,nonce,nonce+1,nonce+2,nonce+3);
  poly_uniform_eta2(&s2.vec[4], buf, nonce+4);
#elif PARAM_K == 6
  poly_uniform_eta2_4x(&s2.vec[0],
	  &s2.vec[1],
	  &s2.vec[2],
	  &s2.vec[3], buf, nonce, nonce + 1, nonce + 2, nonce + 3);
  poly_uniform_eta2(&s2.vec[4], buf, nonce + 4);
  poly_uniform_eta2(&s2.vec[5], buf, nonce + 5);
#endif
...
rej:
...
#elif PARAM_L == 3
	poly_uniform_gamma1m1_3x(&y.vec[0], &y.vec[1], &y.vec[2], &buf[SEEDBYTES],
		nonce, nonce + 1, nonce + 2);
	nonce += 3;
#elif PARAM_L == 4
	poly_uniform_gamma1m1_4x(&y.vec[0], &y.vec[1], &y.vec[2], &y.vec[3], &buf[SEEDBYTES],
		nonce, nonce + 1, nonce + 2, nonce + 3);
	nonce += 4;
#elif PARAM_L == 5
	poly_uniform_gamma1m1_4x(&y.vec[0], &y.vec[1], &y.vec[2], &y.vec[3], &buf[SEEDBYTES],
		nonce, nonce + 1, nonce + 2, nonce + 3);
	poly_uniform_gamma1m1(&y.vec[4], &buf[SEEDBYTES], nonce+4);
	nonce += 5;
#endif 
	...
	polyveck_sub(&tmp, &tmp, &wcs2);//output coefficient  <=4*Q
	polyveck_freeze4q(&tmp);
	if (polyveck_chknorm(&tmp, GAMMA2 - BETA2))
		goto rej;
	...
	//check hw(h)
	polyveck_add(&tmp, &tmp, &ct0);//output coefficient  <=2*Q
